...
MTTLayoutCache::~MTTLayoutCache() {
    if (cachedMeasures != nullptr) {
       free(cachedMeasures);
       cachedMeasures = nullptr;
    }
}

void MTTLayoutCache::cacheResult(MTTSize availableSize, MTTSize resultSize,
	...
	if(layoutAction == LayoutActionLayout) {
        ...
        if (nextMeasureIndex >= cachedMeasureLen && nextMeasureIndex < MAX_MEASURES_COUNT) {
            MeasureResult *ptr = (MeasureResult *)realloc(cachedMeasures, sizeof(MeasureResult) * (nextMeasureIndex + 1));
            if (ptr != nullptr) {
                cachedMeasureLen = nextMeasureIndex + 1;
                cachedMeasures = ptr;
            } else {
                return;
            }
...
static inline bool OldSizeIsUndefinedAndStillFits(MeasureMode sizeMode,
  ...
  return  sizeMode == MeasureModeAtMost &&
		  lastSizeMode == MeasureModeUndefined &&
...
static inline bool NewMeasureSizeIsStricterAndStillValid(MeasureMode sizeMode,
														  ...
														  float lastSize,
														  float lastResultSize) {
  return  lastSizeMode == MeasureModeAtMost &&
		  sizeMode == MeasureModeAtMost &&
          lastSize > size &&
...
MeasureResult* MTTLayoutCache::useMeasureCacheIfPossible(
	...
	for(uint32_t i=0; i < nextMeasureIndex; i++) {
		...
		if(isMeasureNode) {
			if(!widthCanUse) {
				widthCanUse = SizeIsExactAndMatchesOldMeasuredSize(measureMode.widthMeasureMode,
								availableSize.width, cacheMeasure.resultSize.width);
			}

			if(!widthCanUse) {
				widthCanUse = OldSizeIsUndefinedAndStillFits(measureMode.widthMeasureMode, availableSize.width,
								 cacheMeasure.widthMeasureMode, cacheMeasure.resultSize.width);
			}

			if(!widthCanUse) {
				 widthCanUse = NewMeasureSizeIsStricterAndStillValid(measureMode.widthMeasureMode,availableSize.width,
		...
		if(isMeasureNode) {
			if(!heightCanUse) {
				heightCanUse = SizeIsExactAndMatchesOldMeasuredSize(measureMode.heightMeasureMode,
								availableSize.height, cacheMeasure.resultSize.height);
			}

			if(!heightCanUse) {
				heightCanUse = OldSizeIsUndefinedAndStillFits(measureMode.heightMeasureMode, availableSize.height,
...
MeasureResult* MTTLayoutCache::getCachedMeasureResult(MTTSize availableSize, MTTSizeMode measureMode,
													 FlexLayoutAction layoutAction,
													 bool isMeasureNode) {
	if(isMeasureNode) {
...
void MTTLayoutCache::initCache() {
    ...
    cachedMeasureLen = 2;
    cachedMeasures = (MeasureResult *)malloc(sizeof(MeasureResult) * cachedMeasureLen);
    assert(cachedMeasures);
