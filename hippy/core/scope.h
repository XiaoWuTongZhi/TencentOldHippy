...
class Scope {
 public:
  ...
  void AddModuleClass(const std::string& name, std::unique_ptr<ModuleBase> module);
  ...
  std::shared_ptr<CtxValue> RunJS(const uint8_t* data, size_t len, const char* name = nullptr);
 ...
 private:
  ...
  std::unordered_map<std::string, std::unique_ptr<ModuleBase>> module_class_map_;
