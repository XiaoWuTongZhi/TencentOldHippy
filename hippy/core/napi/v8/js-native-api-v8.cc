...
void GetInternalBinding(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  if (!scope) {
    HIPPY_LOG(hippy::Fatal, "GetInternalBinding scope error_");
    info.GetReturnValue().SetUndefined();
    return;
  }
  ...
  for (const auto& fn : module_class->second) {
    ...
    v8::Handle<v8::FunctionTemplate> function_template =
        v8::FunctionTemplate::New(isolate, JsCallbackFunc,
...
void HandleUncaughtJsError(v8::Local<v8::Message> message,
  ...
  if (isolate == nullptr) {
    HIPPY_LOG(hippy::Error, "HandleUncaughtJsError isolate is null");
    return;
  }
...

  {
	...
	HIPPY_DLOG(hippy::Debug, "description_stream = %s", description_stream.str().c_str());
  }
  ...
  if (!trace.IsEmpty()) {
    ...
    for (int i = 0; i < len; ++i) {
      ...
      stack_stream << stack_script_name << ":"
  ...
  // send error_ to js callback if exist
  ...
  if (!cached_data.IsEmpty()) {
    ...
    HIPPY_DCHECK_WITH_MSG(
        runtime,
        "HandleUncaughtJsError runtime is nullptr");
    if (!runtime) {
      HIPPY_LOG(hippy::Error, "handle_uncaught_js_error ctx is nullptr");
      context->SetEmbedderData(0, v8::External::New(isolate, (void*)runtime));
      return;
    }
    std::shared_ptr<Ctx> ctx = nullptr;
    std::shared_ptr<Scope> scope = runtime->scope_;
    if (scope) {
      ctx = scope->GetContext();
    }
    if (!ctx) {
      HIPPY_LOG(hippy::Error, "HandleUncaughtJsError ctx is nullptr");
      return;
    }
    ...
    std::shared_ptr<CtxValue> js_obj =
        ctx->CreateObject(json_str.c_str());

    if (js_obj == nullptr) {
      HIPPY_LOG(hippy::Error, "HandleUncaughtJsError parse json error_, description_stream = %s",
    ...
    std::shared_ptr<CtxValue> ret_value =
        ctx->CallFunction(function, 2, args);
...
void V8VM::RegisterUncaughtExceptionCallback() {
  v8::HandleScope handle_scope(isolate_);
  isolate_->AddMessageListener(
      HandleUncaughtJsError,
      v8::External::New(isolate_, reinterpret_cast<void*>(isolate_)));
}
...
bool V8Ctx::RegisterGlobalInJs() {
  ...
  return global->Set(v8::String::NewFromUtf8(isolate_, "global"),
                           global);
}

bool V8Ctx::SetGlobalVar(const std::string& name,
                                 const char* json) {
  HIPPY_DLOG(hippy::Debug, "SetGlobalVar name = %s, json = %s", name.c_str(), json);
  v8::HandleScope handle_scope(isolate_);
  v8::Handle<v8::Context> context = context_persistent_.Get(isolate_);
  v8::Context::Scope context_scope(context);
  v8::Local<v8::Object> global = context->Global();
  ...
  if (!json_value.IsEmpty()) {
    return global->Set(v8::String::NewFromUtf8(isolate_, name.c_str()), json_value);
  }
  return false;
}
...
void V8Ctx::RegisterNativeBinding(const std::string& name,
  ...
  v8::Handle<v8::Context> context = context_persistent_.Get(isolate_);
  v8::Context::Scope context_scope(context);
  context->SetEmbedderData(0, v8::External::New(isolate_, data));
  //to do
  ...
  context->Global()->Set(
      context, v8::String::NewFromUtf8(isolate_, name.c_str()),
                fn_template->GetFunction()).ToChecked();
}
...
std::shared_ptr<CtxValue> V8Ctx::EvaluateJavascript(
  ...
  HIPPY_DLOG(hippy::Debug, "EvaluateJavascript js_file_name = %s", js_file_name);
  ...
  if (js_file_name) {
    ...
    bool debug = false;
    if (debug) {  // fix bug: file source not show at the first time on dev tools
      file_name =
          std::string("hippy-core-debug:///internal_") + std::string(js_file_name);
    } else {
      file_name =
...
bool V8Ctx::RunScriptWithCache(std::unique_ptr<std::vector<char>> script,
  ...
  HIPPY_DLOG(hippy::Debug, "V8Ctx::RunScriptWithCache file_name = %s, is_use_code_cache = %d",
...
std::shared_ptr<CtxValue> V8Ctx::GetJsFn(std::string& name) {
