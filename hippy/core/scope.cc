...
Scope::Scope(Engine* engine, const std::string& name, 
	std::unique_ptr<RegisterMap> map): engine_(engine), context_(nullptr), 
	name_(name), map_(std::move(map)) {}
...
void Scope::Initialized() {
  ...
  RegisterMap::const_iterator it = map_->find(hippy::base::kContextCreatedCBKey);
  if (it != map_->end()) {
  ...
  std::shared_ptr<CtxValue> function = context_->EvaluateJavascript(source_code.data_, 
	  source_code.length_, "bootstrap.js");
  ...
  std::shared_ptr<CtxValue> internal_binding_fn = hippy::napi::GetInternalBindingFn(self);
...
std::shared_ptr<hippy::napi::CtxValue> Scope::GetModuleValue(const std::string& moduleName) {
...
std::shared_ptr<CtxValue> Scope::RunJS(const uint8_t* data, size_t len, const char* name) {
    std::promise<std::shared_ptr<CtxValue>> promise;
    std::future<std::shared_ptr<CtxValue>> future = promise.get_future();
    JavaScriptTask::Function cb = hippy::base::MakeCopyable([data, len, name, context = context_, p = std::move(promise)]() mutable {
