...
std::unique_ptr<std::vector<char>> HippyFile::ReadFile(
  ...
  if (!file.fail()) {
    ...
    int size = file.tellg();
    if (is_auto_fill) {  // Hippy场景 JSC需要末尾补结束符，此处多分配一个字节，避免之后的操作拷贝
      size += 1;
    }
...
int HippyFile::RmFullPath(std::string dir_full_path) {
  HIPPY_LOG(hippy::Debug, "RmFullPath dir_full_path = %s", dir_full_path.c_str());
...
uint64_t HippyFile::GetFileModifytime(const std::string& file_path) {
  HIPPY_LOG(hippy::Debug, "GetFileModifytime file_path = %s", file_path.c_str());
