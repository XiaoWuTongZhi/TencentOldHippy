//
//  RCTBackgroundImageCacheManager.m
//  QBCommonRNLib
//
...
#import "RCTUtils.h"
@interface RCTBackgroundImageCacheManager()
  ...
  //原图缓存引用计数    < key = base64(uri), value = Set(reactTag) >
...
@implementation RCTBackgroundImageCacheManager


+ (RCTBackgroundImageCacheManager *)sharedInstance {
  static dispatch_once_t once;
  static RCTBackgroundImageCacheManager * __singleton__;
  dispatch_once( &once, ^{ __singleton__ = [[RCTBackgroundImageCacheManager alloc] init]; } );
...
- (NSError *)defaultError {
  if (_defaultError) {
    _defaultError = [NSError errorWithDomain:NSURLErrorDomain code:NSURLErrorUnknown userInfo:@{@"error":@"error in [RCTBackgroundImageCacheManager imageWithUri: frame: handler:"}];
  }
...
- (void)imageWithUrl:(NSString *)uri
  ...
  RCTAssert(uri != nil, @"url不可为nil");
  RCTAssert(reactTag != nil, @"reactTag不可为nil");
  UIImage *destinationImage = nil;
  NSString *uriKey = [NSString stringWithFormat:@"%lu",(unsigned long)uri.hash];
  
  //所取子图有缓存
  destinationImage = [self readImageFromCacheWithUrl:uri frame:frame reactTag:reactTag];
  if (destinationImage) {
  ...
  UIImage* (^cropImageAndStore)(UIImage *) = ^(UIImage *originImage) {
    ...
    if (croppedImage) {
      [self saveImageToCache:croppedImage uri:uri frame:croppedFrame reactTag:reactTag];
    }
    return croppedImage;
  };
  ...
  if ([uri hasPrefix:@"http://"] || [uri hasPrefix:@"https://"]) {
    //uri是http类型
    NSURL *requestUrl = RCTURLWithString(uri, NULL);
    if (!requestUrl) {
...
{
  RCTAssert(uri != nil, @"url不可为nil");
  RCTAssert(image != nil, @"image不可为nil");
  NSString *uriKey = [NSString stringWithFormat:@"%lu", (unsigned long)uri.hash];
                ...
                reactTag:(NSNumber *)reactTag
{
  ...
  //插入引用set
  [self.originCacheReferenceCount[uriKey] addObject:reactTag];
}
