...
#import "RCTBackgroundImageCacheManager.h"
...
- (UIView *)nextResponseViewAtPoint:(inout CGPoint *)point {
    UIView *superView = [self superview];
    if (superView && self.reactTag) {
        ...
        if (0 != index) {
            for (NSInteger i = index - 1; i >= 0 ; i--) {
                ...
                if (pointInside) {
                    ...
                    UIView *returnView = hitTestView ? hitTestView : siblingView;
                    *point = [self convertPoint:*point toView:returnView];
                    return returnView;
                }
            }
        }
...
@implementation RCTView
{
  ...
  RCTBackgroundImageCacheManager *_backgroundCachemanager;
}
...
- (instancetype)initWithFrame:(CGRect)frame
{
  if ((self = [super initWithFrame:frame])) {
    ...
    _backgroundCachemanager = RCTBackgroundImageCacheManager.sharedInstance;
...
- (UIView *)hitTest:(CGPoint)point withEvent:(UIEvent *)event
{
  ...
  switch (_pointerEvents) {
    ...
    case RCTPointerEventsBoxNone:
      return hitSubview;
    default:
          RCTLogError(@"Invalid pointer-events specified %ld on %@", (long)_pointerEvents, self);
      return hitSubview ?: hitView;
  }
}
...
void RCTBoarderColorsRetain(RCTBorderColors c)
{
    if(c.top)
    {
        CGColorRetain(c.top);
...
void RCTBoarderColorsRelease(RCTBorderColors c)
{
    if(c.top)
    {
...
- (void)displayLayer:(CALayer *)layer
{
    ...
    __weak RCTBackgroundImageCacheManager* weakBackgroundCacheManager = _backgroundCachemanager;
    ...
    UIImage *image = RCTGetBorderImage(self.borderStyle,
                                       theFrame.size,
                                       cornerRadii,
                                       borderInsets,
                                       borderColors,
    ...
    CGRect contentsCenter = ({
        ...
        if (size.width == 0 || size.height == 0) {
            RCTLogError(@"divided by zero in RCTView.m");
            return;
        }
        CGRectMake(
    ...
    void (^setImageBlock)(CALayer * ,UIImage *) = ^(CALayer *imageLayer, UIImage *resultingImage) {
        imageLayer.contents = (id)resultingImage.CGImage;
        
        imageLayer.backgroundColor = NULL;
        
        //  weakLayer.contents = (id)image.CGImage;
        imageLayer.contentsScale = image.scale;
        imageLayer.needsDisplayOnBoundsChange = YES;
        ...
        [self updateClippingForLayer:imageLayer];
        
        if (isResizable) {
            imageLayer.contentsCenter = contentsCenter;
        } else {
            imageLayer.contentsCenter = CGRectMake(0.0, 0.0, 1.0, 1.0);
        }
    };
    if (!_backgroundImageUrl) {
        setImageBlock(layer, image);
    }
    else {
        ...
        dispatch_async(global_hpview_queue(), ^{
            CGRect weakFrame = CGRectMake(backgroundPositionX, backgroundPositionY, theFrame.size.width, theFrame.size.height);
            
            [weakBackgroundCacheManager imageWithUrl:backgroundImageUrl frame:weakFrame reactTag:reactTag handler:^(UIImage *decodedImage, NSError *error) {
                if (error) {
                    RCTLogError(@"weakBackgroundCacheManagerLog %@", error);
                    return;
                }
                ...
                if ([NSThread isMainThread]) {
                    setImageBlock(layer, resultingImage);
                }
                else {
...
- (void)dealloc
{
  ...
  if (self.backgroundImageUrl) {
      ...
      __weak RCTBackgroundImageCacheManager* weakBackgroundImageCacheManager = _backgroundCachemanager;
      NSString *weakBackgroundImageUrl = self.backgroundImageUrl;
      NSNumber *weakReactTag = self.reactTag;
      dispatch_async(global_hpview_queue(), ^{
        [weakBackgroundImageCacheManager releaseBackgroundImageCacheWithUrl:weakBackgroundImageUrl frame:frame reactTag:weakReactTag];
      });
  }
}
