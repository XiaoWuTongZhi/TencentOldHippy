...
@interface RCTTextField() <RCTUITextFieldResponseDelegate>
@end
...
- (instancetype)init
{
  if ((self = [super initWithFrame:CGRectZero])) {
    ...
    _textView = [[RCTUITextField alloc] initWithFrame:CGRectZero];
...
- (void)setAuthCodeAutoFill:(BOOL)authCodeAutoFill {
    _authCodeAutoFill = authCodeAutoFill;
    if (@available(iOS 12.0, *) && _authCodeAutoFill) {
...
- (void)textFieldDidChange
{
    ...
    if (!self.reactTag || !_onChangeText) {
        return;
    }
...
- (void)observeValueForKeyPath:(NSString *)keyPath
                      ofObject:(__unused RCTTextField *)textField
