...
static MTTSize x5MeasureFunc (MTTNodeRef node, float width, MeasureMode widthMeasureMode,
  ...
  RCTShadowText *shadowText = (__bridge RCTShadowText *)MTTNodeGetContext(node);
...
static void resetFontAttribute(NSTextStorage *textStorage) {
    ...
    if (0 != numberOfGlyphs) {
        ...
        while (1 == lineFramentEffectiveRange.length) {
            //首先计算最后一个字符是不是单独占了一行,如果是，则通过减少字间距以及字体大小来压缩空间
																					...
																					parentProperties:(NSDictionary<NSString *, id> *)parentProperties
{
	...
	[virtualApplierBlocks addObject: ^(NSDictionary<NSNumber *, RCTVirtualNode *> *virtualRegistry) {
		RCTVirtualTextNode *node = (RCTVirtualTextNode *)virtualRegistry[self.reactTag];
             ...
             absolutePosition:(CGPoint)absolutePosition
{
  @try {
    ...
    [layoutManager.textStorage enumerateAttribute:RCTShadowViewAttributeName inRange:characterRange options:0 usingBlock:^(RCTShadowView *child, NSRange range, __unused BOOL *_) {
      if (child) {
        ...
        CGRect childFrame = {{
          x5RoundPixelValue(glyphRect.origin.x),
          x5RoundPixelValue(glyphRect.origin.y + glyphRect.size.height - height + font.descender)
        }, {
          x5RoundPixelValue(width),
          x5RoundPixelValue(height)
        }};
...
{
  // MttRN: https://github.com/facebook/react-native/issues/11412
  if (isnan(width))
  {
		width = 0;
  }
                                                ...
                                                opacity:(CGFloat)opacity
{
  ...
  UIFont *font = [RCTFont updateFont:f
...
{
  [attributedString enumerateAttribute:attribute inRange:NSMakeRange(0, attributedString.length) options:0 usingBlock:^(id value, NSRange range, __unused BOOL *stop) {
                      ...
                      heightOfTallestSubview:(CGFloat)heightOfTallestSubview
{
  ...
  [attributedString enumerateAttribute:NSParagraphStyleAttributeName inRange:(NSRange){0, attributedString.length} options:0 usingBlock:^(id value, __unused NSRange range, __unused BOOL *stop) {
    if (value) {
  ...
  if (hasParagraphStyle) {
    ...
    if (lineHeight > fontLineHeight) {
      [attributedString addAttribute:NSBaselineOffsetAttributeName
                               value:@(newLineHeight / 2 - maximumFontLineHeight / 2)
                               range:(NSRange){0, attributedString.length}];
    }
  }
