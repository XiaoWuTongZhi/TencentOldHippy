...
extern const NSTimeInterval kRCTAnimatedImageDelayTimeIntervalMinimum;

//
//  An `RCTAnimatedImage`'s job is to deliver frames in a highly performant way and works in conjunction with `RCTAnimatedImageView`.
...
//
@interface RCTAnimatedImage : NSObject
...
// Pass either a `UIImage` or an `RCTAnimatedImage` and get back its size
...
// On success, the initializers return an `RCTAnimatedImage` with all fields initialized, on failure they return `nil` and an error will be logged.
...
@property (nonatomic, strong, readonly) id<RCTImageProviderProtocol> imageProvider;
@end
...
@interface RCTAnimatedImage (Logging)
...
//[RCTAnimatedImage logStringFromBlock:^NSString *{ return [NSString stringWithFormat:(format), ## __VA_ARGS__]; } withLevel:(logLevel)]
