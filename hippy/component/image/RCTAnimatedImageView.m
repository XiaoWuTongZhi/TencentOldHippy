...
#import "RCTAssert.h"
@interface RCTAnimatedImageView ()
...
- (void)setAnimatedImage:(RCTAnimatedImage *)animatedImage
{
...
{
    // Presision is set to half of the `kRCTAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kRCTAnimatedImageDelayTimeIntervalMinimum;
...
- (void)startAnimating
{
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            RCTWeakProxy *weakProxy = [RCTWeakProxy weakProxyForObject:self];
...
- (void)setRunLoopMode:(NSString *)runLoopMode
{
    if (![@[NSDefaultRunLoopMode, NSRunLoopCommonModes] containsObject:runLoopMode]) {
        RCTAssert(NO, @"Invalid run loop mode: %@", runLoopMode);
...
{
    BOOL isVisible = self.window && self.superview && ![self isHidden] && self.alpha > 0.0 && !CGRectIsEmpty(self.bounds);
