...
const NSTimeInterval kRCTAnimatedImageDelayTimeIntervalMinimum = 0.02;

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `RCTAnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, RCTAnimatedImageDataSizeCategory) {
    RCTAnimatedImageDataSizeCategoryAll = 10,       // All frames permanently in memory (be nice to the CPU)
    RCTAnimatedImageDataSizeCategoryDefault = 75,   // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    RCTAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    RCTAnimatedImageDataSizeCategoryUnsupported     // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, RCTAnimatedImageFrameCacheSize) {
    RCTAnimatedImageFrameCacheSizeNoLimit = 0,                // 0 means no specific limit
    RCTAnimatedImageFrameCacheSizeLowMemory = 1,              // The minimum frame cache size; this will produce frames on-demand.
    RCTAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    RCTAnimatedImageFrameCacheSizeDefault = 5                 // Build up a comfy buffer window to cope with CPU hiccups etc.
};

@interface RCTAnimatedImage ()
...
// The actual type of the object is `RCTWeakProxy`.
@property (nonatomic, strong, readonly) RCTAnimatedImage *weakProxy;
@property (nonatomic, strong) id<RCTImageProviderProtocol> imageProvider;
...
- (NSUInteger)frameCacheSizeCurrent
{
    ...
    if (self.frameCacheSizeMax > RCTAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }
    
    if (self.frameCacheSizeMaxInternal > RCTAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize
{
    if (self == [RCTAnimatedImage class]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:nil usingBlock:^(NSNotification *note) {
            ...
            RCTAssert([NSThread isMainThread], @"Received memory warning on non-main thread");
            ...
            // Note that RCTAnimatedImages can be created on any thread, so the hash table must be locked.
            NSArray *images = nil;
...
- (instancetype)init
{
    RCTAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        RAILog(RAILogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `RCTAnimatedImage`.");
    }
    return animatedImage;
}

- (instancetype)initWithAnimatedImageProvider:(id<RCTImageProviderProtocol>)imageProvider {
    return [self initWithAnimatedImageProvider:imageProvider optimalFrameCacheSize:9 predrawingEnabled:YES];
}

- (instancetype)initWithAnimatedImageProvider:(id<RCTImageProviderProtocol>)imageProvider optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled {
    ...
    if (self) {
        ...
        for (NSUInteger i = 0; i < imageCount; i++) {
            @autoreleasepool {
                ...
                if (frameImage) {
                    ...
                    // Support frame delays as low as `kRCTAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `kDelayTimeIntervalDefault` for legacy compatibility.
                    ...
                    if (delayTimeAtFrame < ((float)kRCTAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                        RAILog(RAILogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kRCTAnimatedImageDelayTimeIntervalMinimum);
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= RCTAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            } else if (animatedImageDataSize <= RCTAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = RCTAnimatedImageFrameCacheSizeDefault;
            } else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = RCTAnimatedImageFrameCacheSizeLowMemory;
            }
        } else {
        ...
        _weakProxy = (id)[RCTWeakProxy weakProxyForObject:self];
...
+ (instancetype)animatedImageWithAnimatedImageProvider:(id<RCTImageProviderProtocol>)imageProvider {
    RCTAnimatedImage *animatedImage = [[RCTAnimatedImage alloc] initWithAnimatedImageProvider:imageProvider];
    return animatedImage;
}
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        // Support frame delays as low as `kRCTAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `kDelayTimeIntervalDefault` for legacy compatibility.
                        // To support the minimum even when rounding errors occur, use an epsilon when comparing. We downcast to float because that's what we get for delayTime from ImageIO.
                        if ([delayTime floatValue] < ((float)kRCTAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            RAILog(RAILogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kRCTAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
...
{
    RCTAnimatedImage *animatedImage = [[RCTAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache
{
    ...
    RCTAnimatedImage * __weak weakSelf = self;
...
+ (CGSize)sizeForImage:(id)image
{
    ...
    if ([image isKindOfClass:[UIImage class]]) {
    ...
    } else if ([image isKindOfClass:[RCTAnimatedImage class]]) {
        ...
        RAILog(RAILogLevelError, @"`image` isn't of expected types `UIImage` or `RCTAnimatedImage`: %@", image);
    }
...
{
    self.frameCacheSizeMaxInternal = RCTAnimatedImageFrameCacheSizeNoLimit;
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification
{
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(RCTAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    RAILog(RAILogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)RCTAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = RCTAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(RCTAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
...
@implementation RCTAnimatedImage (Logging)
...
#pragma mark - RCTWeakProxy

@interface RCTWeakProxy ()
...
// This is the designated creation method of an `RCTWeakProxy` and
// as a subclass of `NSProxy` it doesn't respond to or need `-init`.
+ (instancetype)weakProxyForObject:(id)targetObject
{
    RCTWeakProxy *weakProxy = [RCTWeakProxy alloc];
