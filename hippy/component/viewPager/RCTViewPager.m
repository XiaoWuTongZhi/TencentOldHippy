...
- (void)insertReactSubview:(UIView *)view atIndex:(NSInteger)atIndex
{
    if (atIndex > self.viewPagerItems.count) {
        RCTLogWarn(@"Error In RCTViewPager: addSubview —— out of bound of array");
        return;
    }
    
    [super insertReactSubview:view atIndex:(  NSInteger)atIndex];
...
- (void)setPage:(NSInteger)pageNumber animated:(BOOL)animated {
    if (pageNumber >= self.viewPagerItems.count || pageNumber < 0) {
        RCTLogWarn(@"Error In ViewPager setPage: pageNumber invalid");
        return;
    }
...
- (void)refreshViewPager:(BOOL)needResetToInitialPage invokeOnPageSelectd:(BOOL)invokeOnPageSelectd {
    ...
    if (self.initialPage >= self.viewPagerItems.count) {
        RCTLogWarn(@"Error In RCTViewPager: layoutSubviews");
    ...
    if (!lastViewPagerItem) {
        RCTLogWarn(@"Error In RCTViewPager: addSubview");
        self.contentSize = CGSizeZero;
        return;
    }
...
- (NSUInteger)nowPage {
    ...
    if (thePage < 0) {
        RCTLogWarn(@"Error In ViewPager nowPage: thePage invalid");
        return 0;
    } else {
