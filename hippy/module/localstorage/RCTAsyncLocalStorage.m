...
NSString *const RCTStorageDirectory = @"RCTAsyncLocalStorage_V1";
...
@implementation RCTAsyncLocalStorage
{
    ...
    BOOL _RCTHasCreatedStorageDirectory;
}
@synthesize bridge = _bridge;
RCT_EXPORT_MODULE(AsyncStorage)
...
+ (void)clearAllData
{
    ...
    NSString *storageDirectory = [docDir stringByAppendingPathComponent:RCTStorageDirectory];
...
- (NSDictionary *)RCTDeleteStorageDirectory
{
    NSError *error;
    [[NSFileManager defaultManager] removeItemAtPath:[self RCTGetStorageDirectory] error:&error];
    _RCTHasCreatedStorageDirectory = NO;
    return error ? RCTMakeError(@"Failed to delete storage directory.", error, nil) : nil;
}

- (NSString *) RCTGetManifestFilePath
{
    if (_manifestFilePath == nil) {
        _manifestFilePath = [[self RCTGetStorageDirectory] stringByAppendingPathComponent:RCTManifestFileName];
    }
...
- (void)invalidate
{
    if (_clearOnInvalidate) {
        ...
        [self RCTDeleteStorageDirectory];
    }
...
- (NSString *)_filePathForKey:(NSString *)key
{
    ...
    return [[self RCTGetStorageDirectory] stringByAppendingPathComponent:safeFileName];
}
...
- (NSDictionary *)_ensureSetup
{
    ...
    NSError *error = nil;
    if (!_RCTHasCreatedStorageDirectory) {
        [[NSFileManager defaultManager] createDirectoryAtPath:[self RCTGetStorageDirectory]
        ...
        _RCTHasCreatedStorageDirectory = YES;
    }
    if (!_haveSetup) {
        NSDictionary *errorOut;
        NSString *serialized = RCTReadFile([self RCTGetManifestFilePath], nil, &errorOut);
...
- (NSDictionary *)_writeManifest:(NSMutableArray<NSDictionary *> **)errors
{
    ...
    [serialized writeToFile:[self RCTGetManifestFilePath] atomically:YES encoding:NSUTF8StringEncoding error:&error];
    NSDictionary *errorOut;
    if (error) {
...
- (NSString *) RCTGetStorageDirectory
{
    if (nil == _storageDirectory) {
        ...
        /* RCTAsyncLocalStorage负责将配置文件写入本地磁盘
         * 在调用者指定[RCTBridge moduleName]属性的情况下，RCTAsyncLocalStorage会在RCTStorageDirectory目录下依据[RCTBridge moduleName]返回值再次建一个文件夹用于存放当前业务配置。
         * 保证各业务之间数据独立互不共享。
         * !!!请在初始化RCTRootView的时候务必传入moduleName参数!!!。
         * 若调用者不指定[RCTBridge moduleName]，RCTAsyncLocalStorage会将配置写入公共存储文件RCTStorageDirectory中，这样会造成如下问题：
         * 1.多业务同时写入一个文件造成崩溃。 2.多业务写配置文件时若key值相同造成数据错乱。
         */
        RCTAssert(_bridge.moduleName, @"重要！！这里一定要给RCTBridge.moduleName属性赋值。");
        _storageDirectory = [[_storageDirectory stringByAppendingPathComponent:RCTStorageDirectory] stringByAppendingPathComponent:_bridge.moduleName];
    }
...
RCT_EXPORT_METHOD(clear:(RCTResponseSenderBlock)callback)
{
    [_manifest removeAllObjects];
    [RCTGetCache() removeAllObjects];
    NSDictionary *error = [self RCTDeleteStorageDirectory];
