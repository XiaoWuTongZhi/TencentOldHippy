...
- (void)start
{
    ...
    dispatch_queue_t bridgeQueue = dispatch_queue_create("com.tencent.hippy.RCTBridgeQueue", DISPATCH_QUEUE_CONCURRENT);
...
- (void)initModulesWithDispatchGroup:(__unused dispatch_group_t)dispatchGroup
{
    ...
    if (!_javaScriptExecutor) {
        id<RCTJavaScriptExecutor> executorModule = [[self.executorClass alloc] initWithExecurotKey:self.executorKey bridge:self];
        ...
        _javaScriptExecutor = (id<RCTJavaScriptExecutor>)[moduleData instance];
    }
    //RCT_PROFILE_END_EVENT(RCTProfileTagAlways, @"");
...
- (void)executeSourceCode:(NSData *)sourceCode
{
    ...
    [self->_performanceLogger markStartForTag: RCTExecuteSource];
...
//        NSRunLoop *targetRunLoop = [self->_javaScriptExecutor isKindOfClass:[RCTJSCExecutor class]] ? [NSRunLoop currentRunLoop] : [NSRunLoop mainRunLoop];
        ...
        [self->_performanceLogger markStopForTag: RCTExecuteSource];
        // Perform the notification on the main thread, so we can't run into
        // timing issues with RCTRootView
        
#ifdef DEBUG
        NSUInteger RCTPLScriptDownloads = (NSUInteger)[self->_performanceLogger durationForTag:RCTPLScriptDownload];
        NSUInteger RCTPLNativeModuleInits = (NSUInteger)[self->_performanceLogger durationForTag: RCTPLNativeModuleInit];
        NSUInteger RCTPLJSCExecutorSetups = (NSUInteger)[self->_performanceLogger durationForTag: RCTPLJSCExecutorSetup];
        NSUInteger RCTPLNativeModuleInjectConfigs = (NSUInteger)[self->_performanceLogger durationForTag: RCTPLNativeModuleInjectConfig];
        NSUInteger RCTPLNativeModulePrepareConfigs = (NSUInteger)[self->_performanceLogger durationForTag: RCTPLNativeModulePrepareConfig];
        NSUInteger RCTExecuteSources = (NSUInteger)[self->_performanceLogger durationForTag: RCTExecuteSource];
        NSUInteger RCTPLBridgeStartups = (NSUInteger)[self->_performanceLogger durationForTag: RCTPLBridgeStartup];
        
        NSLog(@"common cost: read source:%@ init module:%@ executor setups:%@ inject config:%@ prepare config:%@ load source:%@ bridge starup:%@", @(RCTPLScriptDownloads), @(RCTPLNativeModuleInits), @(RCTPLJSCExecutorSetups), @(RCTPLNativeModuleInjectConfigs), @(RCTPLNativeModulePrepareConfigs), @(RCTExecuteSources), @(RCTPLBridgeStartups));
#endif
...
- (NSDictionary *)RCTExportedDimensions:(BOOL )rotateBounds
{
...
- (NSString *)deviceInfo {
    ...
    [deviceInfo setValue:_RCTSDKVersion forKey:@"SDKVersion"];
    [deviceInfo setValue: _parentBridge.appVerson forKey:@"AppVersion"];
    [deviceInfo setValue:[self RCTExportedDimensions:rotateBounds] forKey:@"Dimensions"];
    
    NSError *error = nil;
    NSData *data = [NSJSONSerialization dataWithJSONObject:deviceInfo options:0 error:&error];
    if (error) {
        NSString *errorString = [NSString stringWithFormat:@"device parse error:%@, deviceInfo:%@", [error localizedFailureReason], deviceInfo];
        NSError *error = RCTErrorWithMessageAndModule(errorString, self.moduleName);
        RCTFatal(error);
    }
...
- (void)handleBuffer:(NSArray *)buffer
{
    ...
    if (RCT_DEBUG && (moduleIDs.count != methodIDs.count || moduleIDs.count != paramsArrays.count)) {
        RCTLogError(@"Invalid data message - all must be length: %lu", (unsigned long)moduleIDs.count);
        return;
    }
                ...
                params:(NSArray *)params
{
    ...
    if (RCT_DEBUG && !moduleData) {
        RCTLogError(@"No module found for id '%lu'", (unsigned long)moduleID);
        return nil;
    }
    
    id<RCTBridgeMethod> method = moduleData.methods[methodID];
    if (RCT_DEBUG && !method) {
        RCTLogError(@"Unknown methodID: %lu for module: %lu (%@)", (unsigned long)methodID, (unsigned long)moduleID, moduleData.name);
        return nil;
    }
    ...
    @catch (NSException *exception) {
        ...
        if (self.parentBridge.useCommonBridge) {
            ...
            error = [[NSError alloc] initWithDomain: RCTErrorDomain code: 0 userInfo: errorInfo];
        }
        RCTFatal(error);
        return nil;
    }
