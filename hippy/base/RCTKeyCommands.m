...
- (void)RCT_handleKeyCommand:(UIKeyCommand *)key
{
    ...
    if (RCTIsIOS8OrEarlier() || CACurrentMediaTime() - lastCommand > 0.5) {
        for (RCTKeyCommand *command in [RCTKeyCommands sharedInstance].commands.allObjects) { //add by stockGroup
                         ...
                         modifierFlags:(UIKeyModifierFlags)flags
{
    ...
    for (RCTKeyCommand *command in _commands.allObjects) { //add by stockGroup
        if ([command matchesInput:input flags:flags]) {
            return YES;
        }
    }
