...
NSString *const RCTBusinessDidLoadNotification = @"RCTBusinessDidLoadNotification";
NSString *const _RCTSDKVersion = @"0.4.9.1";
...
@implementation RCTBridge
{
    ...
    id <RCTImageViewCustomLoader> _imageLoader;
    id <RCTCustomTouchHandlerProtocol> _customTouchHandler;
    NSSet<Class<RCTImageProviderProtocol>> *_imageProviders;
...
- (instancetype)initWithDelegate:(id<RCTBridgeDelegate>)delegate
                  ...
                  moduleProvider:(RCTBridgeModuleProviderBlock)block
                   launchOptions:(NSDictionary *)launchOptions
{
    return [self initWithDelegate:delegate bundleURL:bundleURL moduleProvider:block launchOptions:launchOptions executorKey:nil];
}
...
{
    if ([self isKindOfClass: [RCTBatchedBridge class]]) {
...
- (RCTExtAnimationModule *) animationModule {
    return [self moduleForName:@"AnimationModule"];
}

- (id <RCTImageViewCustomLoader>)imageLoader
{
    if (!_isInitImageLoader) {
        _imageLoader = [[self modulesConformingToProtocol: @protocol(RCTImageViewCustomLoader)] lastObject];
...
- (id <RCTCustomTouchHandlerProtocol>)customTouchHandler
{
    if (!_customTouchHandler) {
        _customTouchHandler = [[self modulesConformingToProtocol:@protocol(RCTCustomTouchHandlerProtocol)] lastObject];
    }
    return _customTouchHandler;
}

- (NSSet<Class<RCTImageProviderProtocol>> *)imageProviders {
    if (!_imageProviders) {
        NSMutableSet *set = [NSMutableSet setWithCapacity:8];
        for (Class moduleClass in self.moduleClasses) {
            if ([moduleClass conformsToProtocol:@protocol(RCTImageProviderProtocol)]) {
