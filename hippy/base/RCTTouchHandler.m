...
- (BOOL)checkViewBelongToTouchHandler:(UIView *)view {
    ...
    UIView *checkView = [_bridge.uiManager viewForReactTag:reactTag];
    ...
    if (!_bLongClick) {
        ...
        if (_onLongClickView && _onLongClickView.onLongClick) {
            if ([self checkViewBelongToTouchHandler:_onLongClickView]) {
                self->_onLongClickView.onLongClick(@{});
            }
        }
    }
}
                                                       ...
                                                       atPoint:(CGPoint)point
{
    ...
    NSNumber *innerTag = [targetView reactTagAtPoint:point];
    if (innerTag && ![targetView.reactTag isEqual:innerTag]) {
        UIView *innerView = [_bridge.uiManager viewForReactTag:innerTag];
                                                           ...
                                                           atPoint:(CGPoint)point
{
    ...
    while (view) {
        ...
        if ((onInterceptTouchEvent && findActions.count == 0) || [view isKindOfClass: NSClassFromString(@"RCTRootContentView")]) {
            break ;
        } else {
...
- (BOOL)gestureRecognizer:(__unused UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    [touch.view RNLoopSuperViewHierarchy:^(UIView *view, BOOL *stop) {
        ...
        if ([view conformsToProtocol:@protocol(RCTScrollProtocol)])
        {
            if ([(id<RCTScrollProtocol>)view isManualScrolling])
            {
                ret = NO;
                *stop = YES;
            }
...
- (void)cancelTouch
{
    if (_onPressInView) {
        ...
        if (_onPressInView.onPressOut) {
            if ([self checkViewBelongToTouchHandler:_onPressInView]) {
                self->_onPressInView.onPressOut(@{});
            }
        }
    }
    _bLongClick = NO;
