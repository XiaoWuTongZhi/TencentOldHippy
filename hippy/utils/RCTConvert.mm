...
+ (NSIndexSet *)NSIndexSet:(id)json
{
    ...
    for (NSNumber *number in json) {
        ...
        if (RCT_DEBUG && index < 0) {
            RCTLogError(@"Invalid index value %ld. Indices must be positive.", (long)index);
        }
...
+ (NSURL *)NSURL:(id)json
{
    ...
    @try { // NSURL has a history of crashing with bad input, so let's be safe
        ...
        NSURL *URL = RCTURLWithString(path, NULL);
        ...
        if ([path rangeOfString:@":"].location != NSNotFound) {
            ...
            URL = RCTURLWithString((__bridge NSString *)stringRef, NULL);
            CFRelease(urlRef);
            if (URL) {
                return URL;
            }
...
NSNumber *RCTConvertEnumValue(__unused const char *typeName, NSDictionary *mapping, NSNumber *defaultValue, id json)
{
    if (!json) {
        return defaultValue;
    }
...
static void RCTConvertCGStructValue(__unused const char *type, NSArray *fields, NSDictionary *aliases, CGFloat *result, id json)
{
    NSUInteger count = fields.count;
    if ([json isKindOfClass:[NSArray class]]) {
        if (RCT_DEBUG && [json count] != count) {
            RCTLogError(@"Expected array with count %lu, but count is %lu: %@", (unsigned long)count, (unsigned long)[json count], json);
        } else {
...
+ (NSArray<NSArray<NSString *> *> *)NSStringArrayArray:(id)json {
    return RCTConvertArrayValue(@selector(NSStringArray:), json);
}
...
RCT_ENUM_CONVERTER(FlexWrapMode, (@{
                                    @"wrap": @(FlexWrap),
                                    @"nowrap": @(FlexNoWrap)
                                    }), FlexNoWrap, intValue)

RCT_ENUM_CONVERTER(DisplayType, (@{
...
+ (UIImage *)UIImage:(id)json
{
    ...
    NSURL *URL = RCTURLWithString(uri, NULL);
    ...
    if ([scheme isEqualToString:@"file"]) {
        ...
        RCTLogConvertError(json, @"an image. Only base64 image data are supported.");
        return nil;
    }
    return image;
}
