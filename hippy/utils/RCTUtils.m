...
BOOL RCTForceTouchAvailable(void)
{
    ...
    if (@available(iOS 9.0, *)) {
        forceTouchCapability = (RCTKeyWindow() ?: [UIView new]).traitCollection.forceTouchCapability == UIForceTouchCapabilityAvailable;
    }
...
NSError *RCTErrorWithMessageAndModule(NSString *message, NSString *moduleName) {
    NSString *errorMsg = [NSString stringWithFormat:@"for module:%@ error occured:%@ ", moduleName?:@"unknown module", message];
    NSDictionary<NSString *, id> *errorInfo = @{NSLocalizedDescriptionKey: errorMsg};
    return [[NSError alloc] initWithDomain:RCTErrorDomain code:0 userInfo:errorInfo];
}
...
NSURL *__nullable RCTURLWithString(NSString *URLString, NSString *baseURLString) {
    if (URLString) {
        NSURL *baseURL = RCTURLWithString(baseURLString, NULL);
...
#define MSYS_IS_64BIT() ((sizeof(NSInteger) == 8))

BOOL RCTSystemMemoryGreaterThan2GBDevice()
{
    ...
    dispatch_once(&onceToken, ^{
        if(!MSYS_IS_64BIT())
            is = false;
        //是否是>= 2GB内存设备
        NSString *platfrom = PlatformString();
        if(platfrom)
        {
            bool(^check)(NSString*,NSString*,int) = ^(NSString *p,NSString *m,int level){
                
                bool v = NO;
                NSRange r = [m rangeOfString:p];
                if(r.location != NSNotFound)
                {
                    NSString *d = [m substringFromIndex:r.location+r.length];
                    NSArray<NSString*> *arr = [d componentsSeparatedByString:@","];
                    if(arr.count == 2)
                    {
                        v = [[arr firstObject] integerValue] >= level;
                    }
                }
                return v;
            };
            ...
            //iphone>= 6s (iPhone8,1)
            //ipad>= ipad air 2(iPad5,3)
            NSString *model = [platfrom lowercaseString];
            @try {
                is = check(@"iphone",model,8) || check(@"ipad",model,5);
            } @catch (NSException *exception) {
            }
        }
    });
